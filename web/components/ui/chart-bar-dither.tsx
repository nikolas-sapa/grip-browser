...
function formatValue(v: number): string {
  ...
  // One decimal below 100: the registry rounded, which turned a measured 1.5x
  // into a label reading "2" — a rounded ratio is a different claim.
...
export function ChartBarDither({ data = [], title = "Chart", className = "" }: ChartBarDitherProps) {
  ...
  // useId rather than the registry's Math.random ref: random during render is
  // impure and the ref read during render is a second violation. useId is also
  // SSR-stable, which the random id was not.
