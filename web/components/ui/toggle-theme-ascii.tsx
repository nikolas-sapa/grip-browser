...
export function ThemeToggleAscii({
  ...
  useEffect(() => {
    ...
    paint();
    // Mount-only flag: the server cannot know the resolved theme, so the label
    // and aria-pressed stay neutral until the client has painted once.
  ...
  useEffect(() => {
    ...
    // Uncontrolled path only; this page drives the toggle from next-themes, so
    // it never runs here. Reading the real class on mount is the sync point.
    // eslint-disable-next-line react-hooks/set-state-in-effect
